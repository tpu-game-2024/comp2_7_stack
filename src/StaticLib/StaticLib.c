...
bool push(STACK* s, int val)
{
	...
	if (s == NULL || s->stack_pointer == NULL || s->stack_memory >= s->stack_pointer) return false;  
...
bool push_array(STACK* s, int* addr, int num)
{
	...
	if (s == NULL || s->stack_pointer - num < s->stack_memory || addr == NULL || num <=0) return false;

    for (int i = 0; i < num; i++) {
        if (push(s, addr[(num - 1) - i]) == NULL) return false;
    }
    return true;
}
...
int pop_array(STACK* s, int* addr, int num)
{
	...
	// 積んだ要素数を返り値として返そう
	if (s == NULL || s->stack_pointer ==s->end || addr == NULL || num <= 0) return 0;
